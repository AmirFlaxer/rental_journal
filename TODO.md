...
## 🧾 שאריות מודול השקים-החוזרים (נוסף 26/7/2026)

ארבעה פריטים שנותרו פתוחים בסוף הסשן של 26/7. אף אחד אינו חוסם, אבל הם נרשמו כדי
שלא ייעלמו. הועברו לכאן מקובץ-המוקד הגלובלי, שאינו המקום שלהם.

- **`toISOString` בזרימות-תשלום נוספות.** אותו דפוס-תאריך שכבר תוקן במקום אחד חוזר
  בזרימות נוספות, והן לא נסרקו כולן.
- **מסך-החובות אינו מבחין בשק שחזר.** חוב שמקורו בשק-שחזר נראה זהה לחוב רגיל.
- **`bounce/route.ts` אינו אטומי.** רישום-ההחזרה וסגירת-התזכורת אינם בטרנזקציה אחת;
  כשל באמצע ישאיר מצב חלקי.
- **קוד מת:** הפרמטר `title` ב-`closeCheckReminderForPayment` אינו בשימוש.
