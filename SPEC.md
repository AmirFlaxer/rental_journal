...
המשתמש בלוח-הבקרה, ותיקון ה-service worker שלא חוטף עוד ניווטי-מסמך. **פתוח אחד:** הכיתוב
מתחת לאייקון במכשיר עדיין "יומן הספר" - השם נצרב ב-WebAPK בזמן ההתקנה, ולכן נדרשת התקנה-מחדש
של ה-PWA (הסרה מהגדרות-אפליקציות ואז התקנה מכרום). ראה [LESSONS.md] על מלכודת ה-WebAPK.
