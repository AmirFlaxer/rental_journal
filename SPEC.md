...
- **פתוח**: `new Date().toISOString()` במקום `localDateStr` - בין חצות ל-03:00 שעון ישראל מחזיר את היום הקודם. אומת בפועל: תקבול שנרשם ב-26.7 02:37 נשמר עם הוצאת מס בתאריך 25.7. קריטי בגבול חודש (שק שמופקד ב-1.8 בלילה ייפול ליולי בדוח המס). קיים גם מראש ב-`togglePaid` ו-`markVirtualPaid`.
