...
## תיקון: חוזה שיש לו חוזה-המשך לא מופיע כ"מסתיים" (2026-09-23)

- **הבאג:** אמיר הזין חוזה עתידי לנורדאו 58 (מתחיל 8.10.2026), ו"דורש טיפול" המשיך להציג
  "חוזה נורדאו 58 מסתיים - בעוד 14 ימים". `buildAttentionItems` קיבל רק חוזים פעילים היום
  (`leases.filter(isLeaseCurrentlyActive)`), ולכן לא ראה את החוזה העתידי. אותו פער היה גם ב-
  `generateVirtualLeaseRenewalTasks` (תזכורת "סיום חוזה מתקרב"), שבדק כל חוזה לבדו.
- **התיקון:** `hasSuccessorLease` חדש ב-[lease-status.ts](src/lib/lease-status.ts). חוזה נחשב
  מטופל כשיש באותו נכס חוזה אחר, שלא בוטל, שמתחיל אחריו **וגם** מסתיים אחריו. רווח בין
  החוזים עדיין נחשב המשך, וחוזה קודם לא נחשב. הוחל בשני המקומות. `buildAttentionItems` מקבל
  עכשיו `allLeases` כשדה חובה. 10 בדיקות חדשות, 211 בסך הכול. קומיט `434111a`.
- **אומת חי ע"י אמיר:** אחרי רענון, ההתראה על נורדאו 58 נעלמה מהדשבורד.
- **תוקן באותו סשן: מסך "ייבוא חוזה" סגר (`status: "ended"`) כל חוזה פעיל בנכס**, גם כשהחוזה
  המיובא מתחיל אחרי סופו. עכשיו `activeLeasesToArchiveOnImport` סוגר רק חוזים פעילים שחופפים
  לתקופה המיובאת, לפי אותו כלל-חפיפה (כולל גבולות) שבו `POST /api/leases` חוסם. חוזה שמתחיל
  ביום שבו הקודם מסתיים עדיין חופף, ונשאר בהתנהגות הקודמת. גם האזהרה "יועבר לארכיון" בטופס
  מוצגת רק אז. 5 בדיקות, 216 בסך הכול. ה-UI של מסך הייבוא לא אומת ויזואלית (אין session
  מקומי). השינוי בו הוא רק התנאי של אזהרה קיימת.

## שינוי: מודל Ollama ברירת-מחדל ל-qwen3.8:27b (2026-09-23)

- **ברירת-המחדל הקודמת `qwen2.5:7b` לא הייתה מותקנת במחשב של אמיר.** בלי `OLLAMA_MODEL` מוגדר, בחירה ב-Ollama הייתה נכשלת. ה-README תיעד עוד מוקדם יותר את `llama3.2`.
- **השוואה עם הפרומפט האמיתי** (RTX 4090): בשני חוזים סינתטיים (חוזה ונספח), qwen3.8:27b, qwen3.6:27b ו-qwen3.5:9b קיבלו כולם ציון מלא. qwen3.8 מהיר פי 3 מ-3.6 באותו גודל (~130 מול ~46 טוקנים לשנייה). לכן נבחר qwen3.8.
- **נתיב `documents/[id]/extract`** עובד דרך `/v1/chat/completions`, ושם `think:false` נבלע בשקט. qwen3.8 חשב שם ויצר פי 8 טוקנים. נוסף `reasoning_effort: "none"`, ואומת שלא שובר מודל שאינו תומך בחשיבה (devstral). נתיב `extract-temp` כבר מכבה חשיבה דרך `/api/chat` עם `think:false`.
- **לא נבנה: קריאת PDF סרוק דרך ראייה מקומית.** נבדק על חוזה אמיתי סרוק (8 עמודים). הציון הגבוה ביותר היה 14 מתוך 20. כל המודלים לקחו תאריכים מחוזה קודם שמוזכר במבוא, qwen3.6 החליף בין שוכר למשכיר, וריצה חוזרת של qwen3.8 נתקעה בלולאה. סריקות נשארות ב-Gemini.
- **תזכורת:** Ollama לא נגיש לאתר בפרודקשן (Vercel), אלא אם מוגדרת מנהרה. הוא רלוונטי רק בהרצה מקומית.

## תיקון: בהירות פיד "תנועות אחרונות" + תאריך-תשלום נבחר (2026-08-04)

- **לא היה באג-נתונים.** דווח שתשלום "נדד" ממאי ליוני בלוח-הבקרה. אימות ב-`/api/payments`
  (JSON גולמי, אותו session מחובר) הראה שתי רשומות תקינות לאותו חוזה: שכ"ד יוני שולם 25.6,
  ושכ"ד **מאי** שולם באיחור ב-**1.6**. הפיד מקבץ לפי `paid_date` (מתי שולם בפועל), לא
  `due_date` (לאיזה חודש שייך) - ולכן תשלום מאוחר נראה כמו כפילות של החודש הנוכחי.
- **החלטה**: זו התנהגות נכונה חשבונאית (הכסף אכן נכנס ביוני) שדורשת רק בהירות בתצוגה, לא שינוי-קיבוץ.
  - `monthNameHe` חדש ב-[dates.ts](src/lib/domain/dates.ts) - כל שכ"ד בפיד מתויג בחודש-החיוב שלו.
  - עמוד תקבולים: לחיצת "שולם" פותחת שדה תאריך לעריכה (ברירת מחדל היום) במקום לנעול
    `paid_date` תמיד על "עכשיו" - `dateNoonIso` חדש מפריד את חותמת-הצהריים-UTC מ"עכשיו".
- **לא אומת ויזואלית חי**: טופס-תאריך-התשלום החדש בעמוד תקבולים - לא הייתה רשומת "לתשלום"
  אמיתית לבדוק עליה בזמן הסשן, ולא נגעתי בדאטה אמיתי כדי לא לסכן היסטוריית-תשלומים.
  ראה [docs/current_focus.md](docs/current_focus.md).
...
- **שלושת פריטי-הזנב נסגרו 28/7/2026:** (1) **התראה מקדימה לתזכורת שנתית** - תדירות שנתית מקבלת עכשיו תמיד את חודש-העוגן של השנה הנוכחית, בלי תלות באכלוס, ולכן חידוש הביטוח מופיע ב"עתידיות" מרגע ההגדרה ולא רק בחודש עצמו. בדיקה קיימת שדרשה את ההיפך הפכה את כיוונה במפורש. (2) **`annual` בלי `anchor_month` נדחה בשכבת ה-API** (`propertyUtilitySchema`/`propertyUtilityUpdateSchema`, 7 בדיקות) - היה כשל שקט: תזכורת שלא נוצרת אף פעם, בלי שגיאה. (3) **`gen:types` הורץ** - והתברר שהטיפוסים הנוצרים היו מיושנים בשתי טבלאות שלמות (`check_bounces` **לא היה בהם כלל**, `lease_securities` היה חלקי); הטלאי הידני של `anchor_day` הוסר. **אחרי (1) נמחקה גם שורת-הגשר של 31.10.2026 - המסד ירד ל-12 משימות, וכל מה ששרד מהמחולל הישן הוא היסטוריה מסומנת-בוצע.** 198 בדיקות.
- **פתוח (לא חוסם, שאלות-הכרעה ולא באגים)**: חוזה בסטטוס `paused` נחשב "ריק" ומייצר חלון-ריקות מלא - שאלת-דומיין · שני חצים (`→`) בהערות קיימות ב-`tasks/page.tsx` מפרים את כלל ה-RTL ולא נגענו בהם · `anchor_day` תומך בשנתי בלבד, ולכן חשבון דו-חודשי אינו יכול להיות מעוגן ליום-בחודש (זה מה שהוריד את מי אביבים מ-18 ל-1).
...
- AI: Gemini 2.5 Flash (ברירת מחדל) / Claude Opus 4.6 / Ollama qwen3.8:27b
- Vercel (hosting + cron)
...
**נקודת-הכניסה לסשן הבא: [docs/current_focus.md](docs/current_focus.md)** - מה עושים עכשיו,
מה סגור ואין לפתוח, ומה המסוכן. הקובץ הזה (SPEC) גובר עליו בכל סתירה.
