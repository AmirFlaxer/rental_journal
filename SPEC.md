...
**עודכן 26/7/2026.** הנוסח הקודם כאן הכריז "אין כרגע TODO טכני-דחוף פתוח" - **וזה כבר לא נכון.**
שני פריטים ממתינים בפועל, ושניהם נוצרו בסשן של 26/7:

1. 🔴 **הענף `feature/bounced-checks` נדחף אך לא מוזג ל-`main` - כלומר לא פרוס.**
   אמיר ביקש לבדוק במכשיר לפני מיזוג. **שים לב למצב-הביניים:** המיגרציה של `check_bounces`
   **כבר הורצה בפרודקשן**, אבל הקוד שמשתמש בה עדיין לא שם. זה לא מזיק (טבלה ריקה שאיש לא
   קורא ממנה), אבל זה אומר שהמסד והקוד אינם באותו מצב.
   המיזוג: `git checkout main && git merge feature/bounced-checks && git push`.
2. 🟡 **spec כתוב ומאושר שטרם מומש:** `docs/superpowers/specs/2026-07-26-utility-reminders-design.md`
   (ביטוח שנתי · אחריות-לפי-אכלוס · אופק מתגלגל). זה מה שיאפשר למחוק סוף-סוף את 72 תזכורות-2049.
3. 🔵 **טרם אופיין: "עזיבה ופינוי".** הפער שזוהה - `early_term_protection` **יחסום ביטול-חוזה
