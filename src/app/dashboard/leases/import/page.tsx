    ...
    try {
      ...
      if (propertyAction === "use-existing" && matchedProperty) {
        ...
        const newProp = await fetch("/api/properties", {
          ...
          body: JSON.stringify({
            ...
            house_number: data.propertyHouseNumber || undefined,
      ...
      const leaseRes = await fetch("/api/leases", {
        ...
        body: JSON.stringify({
          ...
          lease_term: calcMonths(data.startDate, data.endDate),
          terms: data.terms || undefined,
          second_tenant_first_name: hasSecond && data.secondTenantFirstName ? data.secondTenantFirstName : undefined,
          second_tenant_last_name: hasSecond && data.secondTenantLastName ? data.secondTenantLastName : undefined,
          second_tenant_id_number: hasSecond && data.secondTenantIdNumber ? data.secondTenantIdNumber : undefined,
          second_tenant_phone: hasSecond && data.secondTenantPhone ? data.secondTenantPhone : undefined,
          second_tenant_email: hasSecond && data.secondTenantEmail ? data.secondTenantEmail : undefined,
          payment_method: data.paymentMethod || undefined,
          check_bank: data.checkBank || undefined,
          check_branch: data.checkBranch || undefined,
          check_account: data.checkAccount || undefined,
          check_deposit_reminder: data.checkDepositReminder,
          has_option: data.hasOption || undefined,
          option_months: data.hasOption && data.optionMonths ? Number(data.optionMonths) : undefined,
          option_rent: data.hasOption ? data.optionRent : undefined,
          option_start: data.hasOption && data.optionStartDate ? data.optionStartDate : undefined,
          option_end: data.hasOption && data.optionEndDate ? data.optionEndDate : undefined,
          option_terms: data.hasOption && data.optionTerms ? data.optionTerms : undefined,
        }),
      });
      if (!leaseRes.ok) {
